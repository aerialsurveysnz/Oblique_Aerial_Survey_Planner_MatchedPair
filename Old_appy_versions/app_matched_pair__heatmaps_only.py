...
DEFAULT_SPEED_KTS = 62.0
DEFAULT_SPEED_MS = DEFAULT_SPEED_KTS / 1.94384
OVERLAP_PRESETS = {
...
st.set_page_config(page_title="Oblique Survey Planner (Matched Pair)", page_icon="✈️", layout="wide")
...
alt_input = st.sidebar.number_input(
    ...
    value=round(m_to_unit(DEFAULT_ALTITUDE_M, dist_unit), 1),
...
speed_ms = st.sidebar.number_input(
    "Aircraft speed (m/s)", value=round(DEFAULT_SPEED_MS, 1), min_value=1.0, max_value=300.0, step=0.5
)
...
fwd_pct  = st.sidebar.slider("Forward overlap (%)", 10, 95, int(st.session_state.fwd_pct), key="fwd_pct")
side_pct = st.sidebar.slider("Sidelap (%)", 10, 95, int(st.session_state.side_pct), key="side_pct")
