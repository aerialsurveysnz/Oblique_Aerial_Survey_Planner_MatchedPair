...
def _line_spacing_for_matched_rl(right_sol, left_sol, target_overlap_fraction):
    ...
    achieves the requested sidelap fraction. Falls back to None if no valid solution.
    """
    ...
    # Search across a generous spacing range. Larger spacing lowers overlap.
    lo = 0.0
    hi = max(abs(v) for v in [r_x0, r_x1, l_x0, l_x1]) * 4.0 + 1.0
    ...
    # If even at zero spacing overlap is already below target, cannot meet target.
    if f_lo < 0:
        return None

    # Increase hi until overlap is below target.
    tries = 0
    while f_hi > 0 and tries < 20:
        hi *= 2.0
        f_hi = f(hi)
        if f_hi is None:
            return None
