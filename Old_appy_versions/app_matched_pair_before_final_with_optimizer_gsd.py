...
def apply_optimizer_choice(best_candidate, unit):
    updated_cameras = [dict(c) for c in st.session_state.cameras]
    apply_common_oblique_settings(
        updated_cameras,
        tilt_deg=best_candidate.get("common_oblique_tilt_deg"),
        focal_mm=best_candidate.get("common_oblique_focal_mm"),
    )
    st.session_state.cameras = updated_cameras

    for idx, cam in enumerate(updated_cameras):
        st.session_state[f"en_{idx}"] = bool(cam["enabled"])
        st.session_state[f"lbl_{idx}"] = cam["label"]
        st.session_state[f"body_{idx}"] = cam["body"]
        st.session_state[f"fl_{idx}"] = float(cam["focal_mm"])
        st.session_state[f"tilt_{idx}"] = float(cam["tilt_deg"])
        st.session_state[f"conv_{idx}"] = cam["tilt_conv"]
        st.session_state[f"orient_{idx}"] = cam["orientation"]
        st.session_state[f"axis_{idx}"] = cam["tilt_axis"]

    st.session_state.planner_altitude_m = float(best_candidate["altitude_m"])
    st.session_state.sidebar_alt_input = round(m_to_unit(float(best_candidate["altitude_m"]), unit), 1)
    st.session_state.sidebar_speed_ms = float(best_candidate["speed_ms"])
    st.session_state.sidebar_reciprocal = bool(best_candidate["reciprocal"])
    st.session_state.fwd_pct = int(best_candidate["forward_pct"])
    st.session_state.side_pct = int(best_candidate["side_pct"])
...
if st.sidebar.button("Load scenario"):
    ...
    if sc and "cameras" in sc:
        ...
        st.session_state.sidebar_alt_input = round(m_to_unit(loaded_altitude_m, dist_unit), 1)
...
# ─────────────────────────────────────────────────────────────────────────────
# OPTIMISER — find the lowest-cost configuration that meets client rules
# ─────────────────────────────────────────────────────────────────────────────
...
with range_col3:
    ...
    opt_search_altitude = st.checkbox("Search altitude", value=False, key="opt_search_altitude")
...
if optimizer_results:
    ...
    if best_valid:
        ...
        b3.metric("Outer GSD", f"{best_valid['outer_gsd_cm']:.2f} cm/px")
        ...
        if st.button("Apply best config to planner", key="apply_best_optimiser_config"):
            apply_optimizer_choice(best_valid, dist_unit)
            st.rerun()
    elif best_near_miss:
        ...
        n3.metric("Outer GSD", f"{best_near_miss['outer_gsd_cm']:.2f} cm/px")
