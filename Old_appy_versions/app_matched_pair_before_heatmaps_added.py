...
CUSTOM_BODY_OPTION = "Custom camera"
...
OVERLAP_PRESETS = {
    ...
    "Balanced": {"forward": 75, "side": 45},
    "Regional": {"forward": 70, "side": 35},
    "Open terrain": {"forward": 65, "side": 30},
    "Custom": None,
}
...
st.set_page_config(page_title="Oblique Survey Planner - Matched Pair", page_icon="ASL_Imagery_Icon.png", layout="wide")
...
def list_saved_scenarios():
    folder = scenario_dir()
    return sorted((p.name for p in folder.glob('*.json')), key=str.lower)

def resolve_scenario_path(name_or_path):
    ...
    if direct.exists():
        return direct
    return scenario_path_from_name(Path(raw).stem)
...
CAMERA_WIDGET_PREFIXES = (
    "en_", "lbl_", "body_", "fl_", "tilt_", "conv_", "orient_", "axis_",
    "custom_name_", "sensor_w_", "sensor_h_", "px_w_", "px_h_", "del_"
)

def reset_camera_widget_state():
    for key in list(st.session_state.keys()):
        if key.startswith(CAMERA_WIDGET_PREFIXES):
            del st.session_state[key]
    st.session_state.camera_widget_nonce = st.session_state.get("camera_widget_nonce", 0) + 1


def apply_loaded_scenario_to_state(sc, loaded_path=None, load_target=""):
    loaded_unit = sc.get("dist_unit", "m")
    if loaded_unit not in ("m", "ft"):
        loaded_unit = "m"
    st.session_state.dist_unit = loaded_unit
    st.session_state._dist_unit_prev = loaded_unit
    st.session_state.altitude_input = round(m_to_unit(float(sc.get("altitude_m", DEFAULT_ALTITUDE_M)), loaded_unit), 1)
    st.session_state.speed_ms_input = float(sc.get("speed_ms", DEFAULT_SPEED_MS))
    st.session_state.reciprocal = bool(sc.get("reciprocal", True))
    st.session_state.fwd_pct = int(round(sc.get("fwd_overlap_pct", st.session_state.get("fwd_pct", OVERLAP_PRESETS["Balanced"]["forward"]))))
    st.session_state.side_pct = int(round(sc.get("sidelap_pct", st.session_state.get("side_pct", OVERLAP_PRESETS["Balanced"]["side"]))))
    loaded_preset = sc.get("overlap_preset")
    if loaded_preset in OVERLAP_PRESETS:
        st.session_state.overlap_preset = loaded_preset
    else:
        st.session_state.overlap_preset = "Custom"
    st.session_state.overlap_preset_last = st.session_state.overlap_preset
    st.session_state.cameras = [ensure_camera_defaults(dict(c)) for c in sc["cameras"]]
    st.session_state.saved_scenario_select = loaded_path.name if loaded_path is not None else ""
    st.session_state.load_scenario_input = str(loaded_path) if loaded_path is not None else load_target
    st.session_state.scenario_name = Path(loaded_path.name).stem if loaded_path is not None else Path(load_target).stem
    reset_camera_widget_state()
    if loaded_path is not None:
        st.session_state._load_success_message = f"Loaded {loaded_path.name}"


def default_custom_body():
    base = BODY_PRESETS["Sony A7R V"]
    return {
        "custom_body_name": CUSTOM_BODY_OPTION,
        "sensor_w_mm": float(base["w_mm"]),
        "sensor_h_mm": float(base["h_mm"]),
        "image_w_px": int(base["w_px"]),
        "image_h_px": int(base["h_px"]),
    }


def ensure_camera_defaults(cam):
    cam.setdefault("enabled", True)
    cam.setdefault("label", "Camera")
    cam.setdefault("body", "Sony A7R V")
    if cam.get("body") == "Custom body":
        cam["body"] = CUSTOM_BODY_OPTION
    cam.setdefault("focal_mm", 50.0)
    cam.setdefault("tilt_deg", 50.0)
    cam.setdefault("tilt_conv", "horiz")
    cam.setdefault("orientation", "portrait")
    cam.setdefault("tilt_axis", "across")
    defaults = default_custom_body()
    for key, value in defaults.items():
        cam.setdefault(key, value)
    return cam


def body_display_name(cam):
    if cam.get("body") == CUSTOM_BODY_OPTION:
        return cam.get("custom_body_name", CUSTOM_BODY_OPTION).strip() or CUSTOM_BODY_OPTION
    return cam.get("body", "Sony A7R V")


def all_body_names():
    names = list(BODY_PRESETS.keys()) + list(load_saved_bodies().keys()) + [CUSTOM_BODY_OPTION]
    seen = set()
    ordered = []
    for name in names:
        if name not in seen:
...
def get_body_for_camera(cam):
    ensure_camera_defaults(cam)
    if cam.get("body") == CUSTOM_BODY_OPTION:
        return {
            "w_mm": float(cam.get("sensor_w_mm", default_custom_body()["sensor_w_mm"])),
            "h_mm": float(cam.get("sensor_h_mm", default_custom_body()["sensor_h_mm"])),
            "w_px": int(cam.get("image_w_px", default_custom_body()["image_w_px"])),
            "h_px": int(cam.get("image_h_px", default_custom_body()["image_h_px"])),
        }
    return get_body(cam.get("body"))
...
def render_app_header(altitude_m, dist_unit, speed_ms, fwd_pct, side_pct):
    ...
    with col_logo:
        if logo_path is not None:
            ...
            st.markdown(
                """
                <div style="padding:0.85rem 1rem;border:1px solid #30363d;border-radius:0.9rem;"
                     >
                    <div style="font-size:0.78rem;color:#8b949e;text-transform:uppercase;letter-spacing:0.09em;">Aerial Surveys</div>
                    <div style="font-size:1.15rem;font-weight:700;color:#e6edf3;">Survey Planning</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    with col_title:
        st.title("Aerial Surveys Oblique Aerial Survey Planner")
        st.caption(
...
EXCEL_HEADER_FILL = PatternFill("solid", fgColor="1F4E78")
EXCEL_SECTION_FILL = PatternFill("solid", fgColor="D9EAF7")
EXCEL_BORDER = Border(
...
def apply_key_value_block(ws, rows, start_row=4):
    ws.cell(start_row, 1, "Item")
    ...
    for offset, (item, value) in enumerate(rows, start=1):
        r = start_row + offset
        ws.cell(r, 1, str(item))
        ws.cell(r, 2, str(value))
        ws.cell(r, 1).fill = EXCEL_SECTION_FILL
        for col in (1, 2):
            ws.cell(r, col).border = EXCEL_BORDER
            ws.cell(r, col).alignment = Alignment(vertical="top", wrap_text=True)

    ws.freeze_panes = f"A{start_row+1}"


def auto_size_columns(ws, extra=2, max_width=45):
    for col_idx, column_cells in enumerate(ws.columns, start=1):
        ...
        ws.column_dimensions[get_column_letter(col_idx)].width = min(max(max_length + extra, 12), max_width)


def add_excel_table(ws, headers, data_rows, start_row, table_name):
    ...
    for row_offset, row in enumerate(data_rows, start=1):
        for col_idx, value in enumerate(row, start=1):
            cell = ws.cell(start_row + row_offset, col_idx, value)
            cell.border = EXCEL_BORDER
...
def add_logo_to_sheet(ws, anchor="H1"):
    logo_path = find_report_logo()
    ...
    try:
        ...
        img.width = 180
        img.height = max(int(img.height * (180 / max(img.width, 1))), 46)
        ws.add_image(img, anchor)
    except Exception:
        pass


def add_png_to_sheet(ws, anchor, png_bytes, width_px=920):
    try:
...
def make_excel_export(settings_rows, system_rows, camera_rows, report_figures=None):
    wb = Workbook()

    ws1 = wb.active
    ws1.title = "Settings"
    style_title(ws1, "Oblique Survey Planner Report", "Survey settings and export-ready values")
    apply_key_value_block(ws1, settings_rows, start_row=4)
    add_logo_to_sheet(ws1)
    auto_size_columns(ws1)

    ws2 = wb.create_sheet("System Summary")
    style_title(ws2, "System Summary", "Coverage, spacing and achieved overlap")
    apply_key_value_block(ws2, system_rows, start_row=4)
    add_logo_to_sheet(ws2)
    auto_size_columns(ws2)

    ws3 = wb.create_sheet("Camera Results")
    style_title(ws3, "Camera Results", "Per-camera geometry, GSD and footprint extents")
    camera_headers = [
        "Camera",
        "Near angle (deg)",
        "Near class",
    ...
    add_excel_table(ws3, camera_headers, camera_rows, start_row=4, table_name="CameraResults")
    add_logo_to_sheet(ws3, anchor="M1")
    auto_size_columns(ws3, max_width=24)

    if report_figures:
        ws4 = wb.create_sheet("Diagrams")
        style_title(ws4, "Coverage Diagrams", "Embedded diagrams from the current app summary")
        add_logo_to_sheet(ws4)
        anchor_rows = [4, 31, 58, 85]
        for idx, (figure_title, figure_bytes) in enumerate(report_figures):
            start_row = anchor_rows[idx] if idx < len(anchor_rows) else anchor_rows[-1] + (idx - len(anchor_rows) + 1) * 27
            ws4.cell(start_row, 1, figure_title)
            ws4.cell(start_row, 1).font = Font(size=12, bold=True, color="1F1F1F")
            ws4.cell(start_row, 1).fill = EXCEL_SECTION_FILL
            ws4.cell(start_row, 1).border = EXCEL_BORDER
            ws4.cell(start_row, 1).alignment = Alignment(horizontal="left")
            add_png_to_sheet(ws4, f"A{start_row+2}", figure_bytes, width_px=860)
...
if "cameras" not in st.session_state:
    st.session_state.cameras = [ensure_camera_defaults(dict(c)) for c in DEFAULT_CAMERAS]
if "dist_unit" not in st.session_state:
    st.session_state.dist_unit = "m"
if "_dist_unit_prev" not in st.session_state:
    st.session_state._dist_unit_prev = st.session_state.dist_unit
if "altitude_input" not in st.session_state:
    st.session_state.altitude_input = round(m_to_unit(DEFAULT_ALTITUDE_M, st.session_state.dist_unit), 1)
if "speed_ms_input" not in st.session_state:
    st.session_state.speed_ms_input = round(DEFAULT_SPEED_MS, 1)
if "reciprocal" not in st.session_state:
    st.session_state.reciprocal = True
if "camera_widget_nonce" not in st.session_state:
    st.session_state.camera_widget_nonce = 0
if "scenario_name" not in st.session_state:
    st.session_state.scenario_name = "my_survey"
if "saved_scenario_select" not in st.session_state:
    st.session_state.saved_scenario_select = ""
if "load_scenario_input" not in st.session_state:
    st.session_state.load_scenario_input = ""
if "_pending_loaded_scenario" not in st.session_state:
    st.session_state._pending_loaded_scenario = None

pending_loaded = st.session_state.get("_pending_loaded_scenario")
if pending_loaded:
    apply_loaded_scenario_to_state(
        pending_loaded["scenario"],
        Path(pending_loaded["loaded_path"]) if pending_loaded.get("loaded_path") else None,
        pending_loaded.get("load_target", ""),
    )
    st.session_state._pending_loaded_scenario = None
...
dist_unit = st.sidebar.selectbox("Distance unit", ["m", "ft"], key="dist_unit")
if st.session_state.get("_dist_unit_prev") != dist_unit:
    prev_unit = st.session_state.get("_dist_unit_prev", "m")
    st.session_state.altitude_input = round(
        m_to_unit(unit_to_m(float(st.session_state.altitude_input), prev_unit), dist_unit), 1
    )
    st.session_state._dist_unit_prev = dist_unit
else:
    st.session_state._dist_unit_prev = dist_unit
...
alt_input = st.sidebar.number_input(
    ...
    min_value=1.0,
    max_value=float(m_to_unit(10000.0, dist_unit)),
    step=float(m_to_unit(10.0, dist_unit)),
    key="altitude_input",
)
altitude_m = unit_to_m(alt_input, dist_unit)

speed_ms = st.sidebar.number_input(
    "Aircraft speed (m/s)", min_value=1.0, max_value=300.0, step=0.5, key="speed_ms_input"
)
st.sidebar.caption(f"≈ {speed_ms * 1.94384:.1f} kts  |  {speed_ms * 3.6:.1f} km/h")
reciprocal = st.sidebar.checkbox("Reciprocal (bidirectional) strips", key="reciprocal")
...
if st.session_state.get("_load_success_message"):
    st.sidebar.success(st.session_state.pop("_load_success_message"))
sc_name = st.sidebar.text_input("Scenario name", key="scenario_name")
if st.sidebar.button("Save scenario"):
    saved_path = save_scenario({
        ...
        "overlap_preset": selected_overlap_preset,
        "reciprocal": reciprocal,
    }, sc_name)
    st.session_state.saved_scenario_select = saved_path.name
    st.session_state.load_scenario_input = str(saved_path)
    st.sidebar.success(f"Saved {saved_path.name}")

saved_scenarios = list_saved_scenarios()
if st.session_state.saved_scenario_select not in ([""] + saved_scenarios):
    st.session_state.saved_scenario_select = ""
scenario_folder = scenario_dir().resolve()
st.sidebar.caption(f"Saved scenarios are stored in: {scenario_folder}")
selected_saved_scenario = st.sidebar.selectbox(
    "Saved scenarios",
    options=[""] + saved_scenarios,
    format_func=lambda x: "Choose a saved scenario..." if x == "" else x,
    key="saved_scenario_select",
)
load_name = st.sidebar.text_input(
    "Or load scenario filename/path",
    key="load_scenario_input",
    help="Use the dropdown above for scenarios saved by this app, or enter a filename like my_survey.json or a full path.",
)
if st.sidebar.button("Load scenario"):
    dropdown_target = (selected_saved_scenario or "").strip()
    typed_target = load_name.strip()
    load_target = dropdown_target or typed_target
    sc, loaded_path = load_scenario(load_target)
    if sc and "cameras" in sc:
        st.session_state._pending_loaded_scenario = {
            "scenario": sc,
            "loaded_path": str(loaded_path) if loaded_path is not None else "",
            "load_target": load_target,
        }
        st.rerun()
    elif not load_target:
        st.sidebar.error("Choose or enter a scenario to load.")
    else:
        target_name = loaded_path.name if loaded_path is not None else load_target
        st.sidebar.error(f"Could not load '{target_name}'")

# ─────────────────────────────────────────────────────────────────────────────
# Main header
# ─────────────────────────────────────────────────────────────────────────────

render_app_header(altitude_m, dist_unit, speed_ms, fwd_pct, side_pct)
st.info("Matched-pair mode: line spacing and achieved sidelap are driven by the reciprocal Right-vs-Left across-track pair where available. If that pair is missing or cannot be solved, the geometry module may fall back to the full-rig swath method.")
st.markdown("---")
...
cameras = [ensure_camera_defaults(cam) for cam in st.session_state.cameras]
...
if b1.button("➕ Add camera"):
    cameras.append(ensure_camera_defaults({
        "enabled": True,
        "label": f"Camera {len(cameras)+1}",
        "body": CUSTOM_BODY_OPTION,
        "focal_mm": 50.0,
        "tilt_deg": 50.0,
        "tilt_conv": "horiz",
        "orientation": "portrait",
        "tilt_axis": "across",
        "custom_body_name": f"Custom camera {len(cameras)+1}",
    }))
    reset_camera_widget_state()
    st.rerun()
...
custom_cameras = [(i, cam) for i, cam in enumerate(cameras) if cam.get("body") == CUSTOM_BODY_OPTION]
if custom_cameras:
    with st.expander("Custom camera sensor details", expanded=True):
        st.caption("Use this when the camera is not one of the saved presets. These values feed the live geometry and exports.")
        for i, cam in custom_cameras:
            st.markdown(f"**{cam['label']}**")
            cc1, cc2, cc3 = st.columns(3)
            cam["custom_body_name"] = cc1.text_input(
                "Camera model/name",
                value=cam.get("custom_body_name", CUSTOM_BODY_OPTION),
                key=f"custom_name_{camera_widget_nonce}_{i}",
            )
            cam["sensor_w_mm"] = float(cc2.number_input(
                "Sensor long axis (mm)",
                min_value=1.0,
                value=float(cam.get("sensor_w_mm", default_custom_body()["sensor_w_mm"])),
                step=0.1,
                key=f"sensor_w_{camera_widget_nonce}_{i}",
            ))
            cam["sensor_h_mm"] = float(cc3.number_input(
                "Sensor short axis (mm)",
                min_value=1.0,
                value=float(cam.get("sensor_h_mm", default_custom_body()["sensor_h_mm"])),
                step=0.1,
                key=f"sensor_h_{camera_widget_nonce}_{i}",
            ))
            cp1, cp2, cp3 = st.columns(3)
            cam["image_w_px"] = int(cp1.number_input(
                "Pixels long axis",
                min_value=100,
                value=int(cam.get("image_w_px", default_custom_body()["image_w_px"])),
                step=100,
                key=f"px_w_{camera_widget_nonce}_{i}",
            ))
            cam["image_h_px"] = int(cp2.number_input(
                "Pixels short axis",
                min_value=100,
                value=int(cam.get("image_h_px", default_custom_body()["image_h_px"])),
                step=100,
                key=f"px_h_{camera_widget_nonce}_{i}",
            ))
            cp3.markdown(
                f"<div style='padding-top:1.9rem;color:#8b949e;'>Pixel pitch updates automatically from the custom sensor and resolution.</div>",
                unsafe_allow_html=True,
            )
            st.markdown("---")
...
for cam, sol, _ in solutions:
    ...
    rows.append({
    "Camera":                    sol.label,
    "Body / FL":                 f"{body_display_name(cam)}  {cam['focal_mm']:.0f} mm",
...
with col_exp1:
    excel_bytes = make_excel_export(settings_rows, system_rows, camera_rows, report_figures=report_figures)
    st.download_button(
        label="Download Excel formatted report",
        data=excel_bytes,
...
|---|---|---|
| Sensor native | — | {body_display_name(cam)} · {bd['w_mm']} × {bd['h_mm']} mm, {bd['w_px']} × {bd['h_px']} px |
